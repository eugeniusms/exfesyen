...
const Button = (props) => {
    return (
        <div>
            { props.preset == "submit" && 
                <button className="bg-latteLight rounded-md border-2 border-navy font-sans px-4 py-1 hover:bg-latteDark hover:drop-shadow-2xl hover:px-6 transition-all ease-out duration-300">
                    { props.children }
                </button> 
            }
            { props.preset == "cancel" && 
